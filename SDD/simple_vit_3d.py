...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, out_dim=None):
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(hidden_dim, out_dim if out_dim is not None else dim),
        )
...
class SimpleViT(nn.Module):
    def __init__(self, *, image_size=(136, 178), image_patch_size=(68, 89), frames=4, frame_patch_size=2, dim, depth=6, heads=8, mlp_dim=512, channels=1, dim_head=64):
        ...
        self.mlp = FeedForward(2048, 512, 6)

    def forward(self, video, target):
        *_, h, w, dtype = *video.shape, video.dtype

        x = self.to_patch_embedding(video)
        print("after patch_emb: ", x.shape)
        ...
        x = self.mlp(x)
        #x = self.to_latent(x)
        #x = self.linear_head(x).view(-1, 6, 4)
        return x


if __name__ == "__main__":
    dataset = TrajDataset("datasets/bookstore/video0", img_step=10)
